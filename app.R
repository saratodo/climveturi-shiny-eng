# Impact of climate change on waterbodies -visualization tool
## Sara Todorovic (sara.todorovic@syke.fi)
## Finnish Environment Institute, ClimVeturi project (2019-2020)

# This app is a translation of the original app (in Finnish)
...
## NOTE ##
# If the input data changes, change the names of locations in this code 
# both in global and server to correspond with the changed names!
...
### First tab with flow rates  ------------
  ...
  output$table1 <- renderReactable({
    ...
    reactable(chg_dfs[[thisName]],
              ...
              columns = list(
                ...
                Virtaama_ilm = colDef(
                  minWidth = 110,
                  header = with_tooltip("Flow rate (\u33a5/s) climate change","Simulated flow rate of the selected climate change scenario during the selected time period."),
  ...
  output$plo <- renderggiraph({
    ...
    m_labels <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
    ...
    plo <- ggplot(
      ...
      labs(title= paste("Simulated flow rates, ", input$location,
                        "\nTime period: ", times[input$timeframe],
                        "\nScenario: ", scens[input$scenario]),
           y = expression(paste("Flow rate (", m^3,"/s)", sep=""))) +
      ...
      scale_colour_manual(name = " ", values = cols,
                          ...
                          labels = c("1981-2010  average",
                                     paste(times[input$timeframe], "average and range", sep = " "))) +
  ...
  output$kuvaaja_lataus <- downloadHandler(
    filename = function() {
      ...
      paste("DischargeGraph_",locs[input$location], "_1981-2010_", times[input$timeframe], "_", 
...
### Second tab with floods  ---------------------
  ...
  output$table2 <- renderReactable({
    ...
    reactable(SharedData$new(flood_list[[tableData]], group ="floods"),
              ...
              columns = list(
                Nimi = colDef(
                  name = "Flow rate measuring point",
                  header = with_tooltip("Flow rate measuring point", "Location of the modelled flow rate measuring point. Click to arrange into alphabetical order.")
                ),
  ...
  output$taulukko2_lataus <- downloadHandler(
    filename = function() {
      ...
      paste("Floods_change%_25scenarios_1981-2010_", times[input$timeframe], ".csv",
            sep = "")
    },
...
ui <- shinyUI(fluidPage(
  ...
  headerPanel(
    ...
    windowTitle = "Impact of climate change on waterbodies"),
  
  titlePanel(h3("Impact of climate change on waterbodies -visualization tool")),
  
  
  # First tab #########
  tabsetPanel(
    tabPanel("Climate change and river flows", fluid = TRUE,
             # Sidebar
             sidebarLayout(
               sidebarPanel(
                 width = 2,
                 ...
                 helpText("Visualize the impacts of climate change on waterbodies over different time periods and scenarios."),
                 
                 selectInput(inputId = "location",
                             label = HTML("Select a location"),
                             choices = locations,selected = ""),
                 bsTooltip("location", "Select the flow rate measuring point to be visualized.", "bottom"),
                 ...
                 bsTooltip("timeframe", "Select one of the future time periods.", "bottom"),
                 ...
                 bsTooltip("scenario", "Select one of the three climate change scenarios. More information about the scenarios can be found in the More information -tab.", "bottom"),
                 
                 # Download plot & table
                 br(),
                 strong("Download links"),
                 div(),
                 downloadLink("kuvaaja_lataus", label = "Download graph (png)"),
                 bsTooltip("kuvaaja_lataus", "Download graph to your device as png.", "bottom"),
                 div(),
                 downloadLink("taulukko1_lataus", label = "Download table (csv)"),
                 bsTooltip("taulukko1_lataus", "Download table to your device as CSV.", "bottom"),
    ...
    tabPanel("Climate change and floods", fluid = TRUE,
             ...
             sidebarLayout(
               sidebarPanel(
                 width = 2,
                 helpText("Visualize the impacts of climate change on 100-year floods (floods occuring statistically once in 100 years) in different time periods."),
                 ...
                 helpText("* the map is still under development. If the map does not refresh after selecting another time period, try refreshing the website again.")
               ),
               ...
               mainPanel(
                 ...
                 #   column(12,
                          ...
                          column(6,
                                 br(),
                                 strong("How much is an average flood every 100 years (1 / 100a) estimated to be affected by climate change?"),
                                 p(" "),
                                 p("The table and map are used to estimate, using 25 different climate change scenarios, how much the 100-year open water flood will change over the selected period relative to the reference period (1981-2010). 'Mean' refers to the average change in 25 different scenarios, with the 'Maximum' being the highest change and the 'Minimum' the minimum change in the scenarios. Note that there is a great deal of uncertainty in the estimates."),
    ...
    tabPanel("More information",
             sidebarLayout(
               sidebarPanel(
                 ...
                 strong("In this page :"),
                 em("background, additional information, contact information and feedback."),
                 helpText("Information about the project: ", 
